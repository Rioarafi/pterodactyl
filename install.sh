...
check_token() {
  ...
  if [ "$USER_TOKEN" = "neroxsikma" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke NEROXXX${NC}"
    echo -e "${YELLOW}TELEGRAM : @NEROXXX${NC}"
    echo -e "${YELLOW}WHATSAPP : 6289530583676${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©NEROXXXOffc${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer NEROXXX   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : NEROXXX      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : NEROXXXOFFC  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by NEROXXXOffc  ${NC}"
